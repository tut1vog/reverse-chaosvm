...
- **The XTEA round constant `0x9E3779B9` (decimal `2654435769`) appears exactly twice in the bytecode.** Verified by counting occurrences in `output/vm-slide/bytecode.json`. This is the same delta used by the register-based `tdc.js` VM's modified-XTEA key derivation (see `docs/CRYPTO_ANALYSIS.md`). Two occurrences of a 32-bit magic number in independently-chosen program data are unlikely to be coincidence; the most probable explanation is that vm-slide contains a parallel XTEA-family construction used by the slide CAPTCHA's verify-body encryption or by an eks-related operation. **Phase 40 task 40.6 will investigate this cross-track finding**, linking vm-slide to the `eks-payload/` and `key-mod/` research tracks.
