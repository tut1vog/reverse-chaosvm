...
| ID | Task | Status |
|----|------|--------|
| 49.1 | Write POST body diff script — reconstruct scraper's Chrome-profile verify POST body (collect + vData + all fields) without live network, compare against committed Puppeteer capture, save diff to `output/phase-49-body-diff/` | in-progress |
| 49.2 | Analysis + fix — based on the diff, identify the causal field(s) and patch the scraper | pending |
...
**ID**: 49.1
**Title**: Write POST body diff script — scraper vs Puppeteer verify POST body comparison
**Phase**: Phase 49 — errorCode -1 root cause
**Status**: in-progress

### Goal
Write a script that reconstructs what the scraper would send in Chrome-profile mode and compares it field-by-field against the committed Puppeteer capture (`output/puppeteer-capture/`). Produces a comprehensive diff of POST fields, decrypted collect cd/sd, and vData plaintext.

### Context
- **Puppeteer reference data** (committed, errorCode 0):
  - `output/puppeteer-capture/verify-post.json` — all 38+ POST fields
  - `output/puppeteer-capture/collect-decrypted.json` — browser section has `cd[60]` + `sd{8 keys}` (XTEA-decrypted). Scraper section is INVALID — it was generated by `decrypt.js` with wrong default sd, ignore it.
  - `output/puppeteer-capture/sd-fields.json` — browser sd with `{od, clientType, coordinate, trycnt, refreshcnt, slideValue, dragobj, ft}`
  - `output/puppeteer-capture/field-mapping.json` — 60-field mapping between browser cd indices and scraper canonical schema indices. Some fields marked `match: false` (captured before Phase 47 Chrome profile fix).
- **Scraper Chrome-profile collect generation**: `tools/scraper/scraper.js` `_generateCollectChrome()` (line 511) loads `profiles/chrome-fingerprint.json` (59-field `cdCanonical`), substitutes per-session fields, reorders via `cdFieldOrder`, places behavioral events at `hashPosition`.
- **Scraper sd**: `tools/scraper/collect-generator.js` `buildSlideSd()` (line 312) — produces `{od, clientType, coordinate, trycnt, refreshcnt, slideValue, dragobj, ft}`. Correct structure, but `coordinate[2]` defaults to `1.0` (browser had `1.8559`), and `ft` is randomly generated.
- **Scraper POST fields**: `tools/scraper/scraper.js` `_buildPostFields()` (line 314) — 38 fields matching captcha-client verify format.
- **Scraper vData**: `tools/vdata-generator/for-post.js` `buildVDataForPost()` — standalone XTEA+base64, 8-field runtime state probe `{tp, key, py, env, version, cLod, inf, ss}`.
- **Puppeteer vData**: in `verify-post.json` field `vData` — 152-char encrypted string. Need to decrypt with the vm-slide XTEA key to compare plaintext. Key is build-specific (`2e430f8c15b7da96` for reference fixtures, may differ for this capture).
- **Template cache**: `tools/scraper/template-cache.js` stores `cdFieldOrder`, `hashPosition`, `serializationDiffs` per template.

### Implementation Steps
1. Write `scripts/diff-verify-bodies.js` that:
   a. Loads the Puppeteer capture data (verify-post.json, collect-decrypted.json browser section, sd-fields.json)
   b. Loads `profiles/chrome-fingerprint.json` and builds the scraper's cdCanonical with per-session substitutions
   c. Reorders cdCanonical per a reference cdFieldOrder (load from template cache or use the committed Puppeteer capture's cd ordering from field-mapping.json)
   d. Builds the scraper's sd via `buildSlideSd()` with the same answer values as the Puppeteer capture
   e. Builds the scraper's POST fields via a mock `_buildPostFields()` using values from the Puppeteer capture's session params
   f. Builds the scraper's vData via `buildVDataForPost()` using the mock POST body
   g. Diffs: POST fields (key-by-key), cd array (index-by-index with field names from COLLECTOR_SCHEMA), sd object (key-by-key), vData (try to decrypt Puppeteer's vData and compare plaintext; if key unknown, compare ciphertext length + structure only)
2. Save output to `output/phase-49-body-diff/diff-report.json`
3. Print a human-readable summary to stdout

### Verification
- [ ] `node scripts/diff-verify-bodies.js` runs without error
- [ ] `output/phase-49-body-diff/diff-report.json` exists with sections: `postFields`, `collectCd`, `collectSd`, `vData`
- [ ] Each section lists matching and differing fields with both values
- [ ] Script correctly identifies Chrome-profile-mode-specific differences (not default profile artifacts)

### Suggested Agent
general-purpose — needs to read multiple source files, understand the scraper's collect pipeline, and write a standalone diff script
