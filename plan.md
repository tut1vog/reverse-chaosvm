...
## Status
Current phase: Phase 44 — vm-slide fingerprint schema + JS vData builder (awaiting user review of post-44.2.7 rescoping 2026-04-15)
Current task: none — 44.2.7 done; Phase 44 downstream rescoping pending user review before next dispatch
...
**Status**: none — 44.2.7 passed verification. Phase 44 downstream tasks need rescoping based on 44.2.7's findings. Director is presenting a rescoping proposal to the user for review; no new task is in-progress until the user approves.

### Pending rescoping proposal (awaiting user review)

Based on 44.2.7's runtime-validated findings:

- **44.4.1** (Math.random comparator in fn 23898): **DROP as unreached**. Runtime entry counts show fn 23898 enters 15× with `(string:3, string:2)` — it's a DOM helper, not the plaintext order source. Retire it with a note; do not remediate.
- **44.3.5** (real-Chrome differential capture): **NARROW**. Third fixture still useful but the capture target simplifies to the 9345-byte POST body + the 152-char vData string pair; we no longer need to reverse fingerprint fields from it because 44.2.7 pinned them live.
- **NEW 44.2.8** (unresolved hop): pin the exact FUNC_CREATE stored in fn 20140's slot 3 / fn 20539's inner slot 8 that directly calls fn 13860 with the rewritten body. Narrow walk of fn 20843 (proxyXHR submodule factory) to find the closure reference. Blocking 44.5b's precise path. Expected ~1–2 hours static. Could alternatively be folded into 44.3.5 or 44.5b — user's call.
- **44.5a** (replay-with-substitution): unchanged; still ships first.
- **44.5b** (from-scratch `build-vdata.js`): unchanged in shape, but now anchored in runtime-verified 8-field schema with known value semantics; no canonical default join order (per 44.4.1's dropped finding + 44.2.7's unknown real source — likely V8 property-enumeration order inside the builder upstream of fn 13860).
- **44.6** (tests): unchanged.
- **44.7** (docs closeout): **EXPAND**. Must correct `docs/CAPTCHA_ORCHESTRATOR.md` §6.2 and §517 (both Phase 42 mechanism and 44.2.6's 9504→112 reduction hypothesis stand — 44.2.7 confirmed body-rewrite injection at pc 20751, with fn 20539 as pure gate+forward), update `CLAUDE.md` Project Memory (8-field schema, tdc-runtime-state-probe framing, runtime key `34e2c8f07b5169ad` vs seed reconciliation), and fold in the runtime/seed key reconciliation owed by 44.5a.

Proposed new dispatch order: **44.2.8 → 44.3.5 → 44.5a → 44.5b → 44.6 → 44.7** (with 44.4.1 retired). Awaiting user confirmation.
